...
add_filter('woocommerce_cart_item_name', function($name, $cart_item, $cart_item_key) {
    if (isset($cart_item['seat_id'])) {
        ...
        $select = '<select class="baxi-tipo-entrada" data-key="' . esc_attr($cart_item_key) . '">'
                . '<option value="adulto"'   . selected($tipo, 'adulto',   false) . '>Adulto + de 18</option>'
                . '<option value="infantil +5"' . selected($tipo, 'infantil', false) . '>Niño + de 5</option>'
                . '<option value="infantil -5"'      . selected($tipo, 'infantil2',      false) . '>Niño - de 5</option>'
                . '</select>';
...
function baxi_get_tipos_entrada() {
    return [
        'adulto'   => ['label'=>'Adulto +18',   'ajuste'=>0],
        'infantil' => ['label'=>'Infantil +5', 'ajuste'=>-8],
        'infantil2'      => ['label'=>'Infantil -5',      'ajuste'=>-15],
    ];
}
    ...
    foreach ($cart->get_cart() as $cart_item) {
        if (!empty($cart_item['tipo_entrada'])) {
            ...
            switch ($cart_item['tipo_entrada']) {
                case 'infantil':
                    $cart_item['data']->set_price($base_price * 0.4667); 
                    break;
                case 'infantil2':
                    $cart_item['data']->set_price($base_price * 0); 
                    break;
                default:
