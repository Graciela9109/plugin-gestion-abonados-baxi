...
class Baxi_Abonados {
    ...
    // NUEVO: función para calcular el precio del abono (titular)
    private function calcular_precio_abono($tipo_abono_id, $extras) {
        global $wpdb;
        // Obtener el registro del tipo de abono y su config
        $row = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}baxi_tipos_abono WHERE id = " . intval($tipo_abono_id));
        if (!$row) return 0;

        $cfg = $row->config ? json_decode($row->config, true) : [];
        $tipo_slug = sanitize_title($row->nombre);
        $precio = floatval($cfg['precio_titular'] ?? 0);
        $precio_nino = floatval($cfg['precio_nino'] ?? 40);
        $precio_nino_menor = floatval($cfg['precio_nino_menor'] ?? 0);
        $precio_nino_club  = floatval($cfg['precio_nino_club'] ?? 0);
        ...
        foreach($extras as $e) {
            $tipo = strtolower($e['tipo_persona'] ?? '');
            if ($tipo === 'nino') $num_nino++;
            if ($tipo === 'nino_menor') $num_nino_menor++;
            if ($tipo === 'nino_club') $num_nino_club++;
        }

        if ($tipo_slug === 'familiar') {
            // Primer niño es gratis (resto pagan precio_nino)
            $niños_pago = max(0, $num_nino - 1);
            $precio += ($niños_pago * $precio_nino);
            // Los nino_menor y nino_club no suman
        } elseif ($tipo_slug === 'familiar-club') {
            // Niño club gratis, resto pagan
            $precio += ($num_nino * $precio_nino);
            // nino_club no suma, nino_menor según config (pero normalmente 0)
        } elseif ($tipo_slug === 'individual' || $tipo_slug === 'juvenil') {
            $precio += ($num_nino * $precio_nino);
        }
        // Extra: añadir precio_nino_menor si hay niños menores y precio es >0
        $precio += ($num_nino_menor * $precio_nino_menor);
        $precio += ($num_nino_club  * $precio_nino_club);
        return $precio;
    }
    ...
    public function guardar_abonado() {
        ...
        // Cambia aquí: el tipo_abono es un ID, no un slug
        ...
        // Extras (recogemos los datos para cálculo)
        $extras_input = [];
        ...
        $precio_abono = $this->calcular_precio_abono($tipo_abono, $extras_input);
        ...
        $data = [
            ...
            'precio_abono'  => $precio_abono
        ];
        $format = array_fill( 0, count( $data ), '%s' );
        // Forzar los numéricos
        $format[ array_search('tipo_abono', array_keys($data)) ] = '%d';
        $format[ array_search('grupo_abono', array_keys($data)) ] = '%d';
        $format[ array_search('precio_abono', array_keys($data)) ] = '%f';
        ...
        if ( isset( $_POST['extras'] ) && is_array( $_POST['extras'] ) ) {
            ...
            foreach ( $_POST['extras'] as $extra ) {
                ...
                $e_data = [
                    ...
                    'precio_abono' => null // extras no tienen precio_abono
                ];
                $wpdb->insert( $tabla, $e_data, array('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%d','%d','%f') );
                $extra_id = $wpdb->insert_id;
                $this->marcar_asiento_abonado( $e['grada'], $e['fila'], $e['asiento'] );
    ...
    // ... El resto igual (renew, eliminar, pdf)
    // (NO MODIFICADO)
