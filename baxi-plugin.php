...
add_filter('woocommerce_add_cart_item_data', function($cart_item_data, $product_id, $variation_id) {
    if (!empty($_GET['baxi_abonado_id'])) {
        $cart_item_data['baxi_abonado_id'] = intval($_GET['baxi_abonado_id']);
    }
...
add_action('woocommerce_before_calculate_totals', function($cart) {
    ...
    foreach ($cart->get_cart() as $cart_item) {
        if (!empty($cart_item['baxi_abonado_id'])) {
            global $wpdb;
            $abonado_id = intval($cart_item['baxi_abonado_id']);
            $precio = $wpdb->get_var($wpdb->prepare(
                "SELECT precio_abono FROM {$wpdb->prefix}baxi_abonados WHERE id = %d", $abonado_id
            ));
            if ($precio !== null && $precio !== false) {
                $cart_item['data']->set_price(floatval($precio));
            }
        }
    }
});
